...
INSERT INTO Patients (disease_id, year_of_birth, gender, referring_doctor, rx_OD, rx_OS, procedures_done, dosage, duration, cumulative_dosage, date_of_discontinuation, extra_notes, merci_rating_left_eye, merci_rating_right_eye)
VALUES 
(2, 1990, 'm', 'Dr. Johnson', 1.0, 1.0, 'Follow-up for RA', 75, 3, 225, NULL, 'Patient has experienced some joint pain', 15, 14);
...
-- Step 6: Create the FAF table
CREATE TABLE IF NOT EXISTS FAF (
    faf_id INT AUTO_INCREMENT PRIMARY KEY,  -- Unique FAF reference ID
    patient_id INT,  -- Foreign key to reference Patients table
    test_id VARCHAR(255) DEFAULT NULL,  -- The TESTID (unique identifier for each test)
    faf_eye ENUM('OD', 'OS') DEFAULT NULL,  -- Eye reference (OD for right eye, OS for left eye)
    image_number INT DEFAULT NULL,  -- Image number associated with the FAF reference
    faf_reference VARCHAR(255) GENERATED ALWAYS AS (CONCAT('FAF/', patient_id, '-', faf_eye, '-', image_number)) STORED,  -- The FAF reference
    date_added TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- Timestamp when the FAF record was added
    FOREIGN KEY (patient_id) REFERENCES Patients(patient_id)  -- Foreign key to link to the Patients table
);

-- Insert sample FAF data
INSERT INTO FAF (patient_id, test_id, faf_eye, image_number)
VALUES 
(1, 'TEST12345', 'OD', 101),  -- FAF for Right Eye of Patient 1
(2, 'TEST67890', 'OS', 102),  -- FAF for Left Eye of Patient 2
(3, 'TEST11223', 'OD', 103),  -- FAF for Right Eye of Patient 3
(4, 'TEST44556', 'OS', 104);  -- FAF for Left Eye of Patient 4

-- Query to view all FAF records and their generated FAF reference
SELECT faf_id, patient_id, test_id, faf_eye, image_number, faf_reference, date_added
FROM FAF;
